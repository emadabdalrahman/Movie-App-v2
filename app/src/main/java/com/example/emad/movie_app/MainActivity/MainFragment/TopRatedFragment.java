...
public class TopRatedFragment extends Fragment implements LoaderManager.LoaderCallbacks<ArrayList<String>>{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View mRootView = inflater.inflate(R.layout.top_rated_movie_fragment, container, false);

        if (isOnline()){
            initializeRecyclerView(mRootView);
            startTopRatedMovieLoader();
        }else {
            ImageView noConnection = (ImageView)mRootView.findViewById(R.id.top_rated_movie_no_connection);
            noConnection.setVisibility(View.VISIBLE);
        }

        return mRootView;
    }
    public void startTopRatedMovieLoader(){
        LoaderManager loaderManager = getActivity().getSupportLoaderManager();
        Loader<ArrayList<String>> loader = loaderManager.getLoader(MOVIE_TOP_RATED_LOADER_ID);
        if (loader == null){
            loaderManager.initLoader(MOVIE_TOP_RATED_LOADER_ID,null,this).forceLoad();
        }else {
            loaderManager.restartLoader(MOVIE_TOP_RATED_LOADER_ID,null,this).forceLoad();
        }
    }
    public void initializeRecyclerView(View view) {
        mRecyclerView = (RecyclerView) view.findViewById(R.id.top_rated_movie_recycler_view);
    ...
    public ArrayList<Movie> getTopRatedMovies(ArrayList<String> mMoviesResponse) {
    ...
    public void onLoadFinished(Loader<ArrayList<String>> loader, ArrayList<String> data) {
        mRecycleViewAdepter = new RecycleViewAdepter(getTopRatedMovies(data), getActivity());
