...
import static com.example.emad.movie_app.Utilities.FinalData.MOVIE_POPULAR_LOADER_ID;

/**
 * Created by emad on 2/17/2017.
 */

public class PopularMovieFragment extends Fragment implements LoaderManager.LoaderCallbacks<ArrayList<String>>{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View mRootView = inflater.inflate(R.layout.popular_movie_fragment, container, false);

        if (isOnline()){
            initializeRecyclerView(mRootView);
            startPopularMovieLoader();
        }else {
            ImageView noConnection = (ImageView)mRootView.findViewById(R.id.popular_movie_no_connection);
    ...
    public void startPopularMovieLoader(){
        LoaderManager loaderManager = getActivity().getSupportLoaderManager();
        Loader<ArrayList<String>> loader = loaderManager.getLoader(MOVIE_POPULAR_LOADER_ID);
        if (loader == null){
            loaderManager.initLoader(MOVIE_POPULAR_LOADER_ID,null,this).forceLoad();
        }else {
            loaderManager.restartLoader(MOVIE_POPULAR_LOADER_ID,null,this).forceLoad();
        }
    }

    public void initializeRecyclerView(View view) {
        mRecyclerView = (RecyclerView) view.findViewById(R.id.popular_movie_recycler_view);
    ...
    public ArrayList<Movie> getPopularMovies(ArrayList<String> mMoviesResponse) {
                ...
                for (int i = 1; i < 2; i++) {
                    try {
                        ...
                        Request mRequest = new Request.Builder()
                                .url("https://api.themoviedb.org/3/movie/popular?api_key=c8e1ac8910c753600986ed6d71f88e29&language=en-US&page=" + i + "")
                                .build();
    ...
    public void onLoadFinished(Loader<ArrayList<String>> loader, ArrayList<String> data) {
        mRecycleViewAdepter = new RecycleViewAdepter(getPopularMovies(data), getActivity());
